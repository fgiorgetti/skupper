...
func NewInterNetworkIngress(ingress *v2alpha1.InterNetworkIngress, links map[string]*ManagementLink) *InterNetworkIngress {
	...
	if mgmtLink, ok := links[inetIngress.LinkName]; ok {
		inetIngress.Link = mgmtLink
	}
	return inetIngress
}

func (m *InterNetworkIngress) Update(desired *v2alpha1.InterNetworkIngress, links map[string]*ManagementLink) bool {
	if desired == nil {
		m.Ingress = nil
		return true
	}
	...
	if mgmtLink, ok := links[m.LinkName]; ok {
		if m.Link == nil {
			m.Link = mgmtLink
			update = true
		}
